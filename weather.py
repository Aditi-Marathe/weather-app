...
def get_forecast(city):
    ...
    if response.status_code == 200:
        ...
        for forecast in data["list"]:
            ...
            forecast_list.append((dt, temp, condition))
...
# Button to check 5-day forecast
if st.button("📅 Show 5-Day Forecast"):
    if city:
        forecast = get_forecast(city)
        if forecast:
            st.markdown("### 🗓️ 5-Day Forecast")
            for day, temp, condition in forecast:
                forecast_emoji = "☀️" if "clear" in condition else "⛅" if "cloud" in condition else "🌧️" if "rain" in condition else "❄️"
                st.markdown(f"""
                    <div class="forecast-box">
                        <p>📅 {day}</p>
                        <p class="weather-details">{forecast_emoji} {condition.capitalize()}</p>
                        <p class="weather-details">🌡️ {temp}°C</p>
                    </div>
                """, unsafe_allow_html=True)
        else:
            st.error("❌ Could not fetch forecast data. Please check the city name or try again later.")
    else:
        st.warning("⚠️ Please enter a city name!")
